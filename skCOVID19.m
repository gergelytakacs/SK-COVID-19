...
disp(['----------------------------'])
disp(['Výhlad na ',datestr(dt),' (do konca d?a):'])
...
disp(['Celkový predpokladaný po?et nakazených: ',num2str(NdSymptoms(max(Day)+1)),' (',num2str(NdSymptomsLow(max(Day)+1)),'-',num2str(NdSymptomsHigh(max(Day)+1)),')'])
disp(['Predpokladaný dátum 100+ overených prípadov: ',datestr(d1+min(find(NdPredicted>100)))])
disp(['Faktor nárastu: ',num2str((gF-1)*100),'%'])
disp(['----------------------------'])
